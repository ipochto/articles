...
<details>
<summary> Так, здесь будет вынужденное отступление от основной темы. А именно, по поводу применения в геймдеве типов активно использующих динамическую аллокациею памяти. </summary>
...
Все животные равны, но некоторые равнее - поэтому добавим шаблон `Custom`, для которого, мало того, что весь список библиотек доступен, так ещё и позволяет загружать их по мере необходимости, а не в момент создания песочницы.
...
Теперь нюнансы, касающиеся самих путей.
...
- Собственно, список разрешённых путей, откуда позволено скрипты запускать.
...
private:
	...
	void loadSafePrint()
    {
    	// В Lua-стейт должна быть звгружена библиотека base, чтобы tostring работал
