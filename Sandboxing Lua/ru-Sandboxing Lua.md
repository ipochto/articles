...
Сама же подмена осуществляется в момент создания Lua-стейта, но на этот раз уже через Си API, а не средствами `sol2`:
```cpp
void * luaAlloc(void *ud, void *ptr, size_t currSize, size_t newSize);
...
{
    size_t used {};
    size_t limit = 8 * 1024 * 1024; // 8 Mb
} alloc_state;

lua_State* L = lua_newstate(lua_alloc, &alloc_state);
sol::state_view lua(L); // Именно не владеющий state_view, а не state.
                        // Но работать мы можем с ним точно так же.
...

// Единственное неудобство - удалять потом придётся вручную.
lua_close(L);
```
